...
class LA_TEA:
    ...
    def foreground_monte_carlo (self,linked_rand_samples,lookup_dict,sht_name,n_iter):
        ...
        =======================================================
        """
        ##initiate results dict: {iter1: {'NPV':xx,'IRR':xxx,...}, iter2: {'NPV':xx,'IRR':xxx,...},...}
        ...
        for iter_ in range(n_iter):
            ...
            for k,v in linked_rand_samples.items():
                ...
                self.temp_sht_cel_new_val_dict[sht_name].append({lookup_dict[k]:abs(v[iter_])*sign})
            #update cell values
            self.update_cells(self.temp_sht_cel_new_val_dict)
            #re-do the intermediate calc
            self.interm_calc()
            #store results of interest
            self.foreground_MC_TEA_results[iter_]={'NPV':self.NPV,'IRR':self.IRR,'min sale price':self.min_sale_pr}
